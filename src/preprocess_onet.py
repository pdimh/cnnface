...
def extract_samples(output_path, pics, sample_type):
    ...
    model_type = ModelType.ONET

    tf.print(
        f'=> Starting Extraction ({model_type}): {sample_type} samples'
    )
            ...
            for sbox in pic_stage2.box:
                ...
                if iou < 0.3 and neg < 3 * (pos + 1):
                    ...
                    preprocessing.save_img(
                        spic, output_path, model_type, sample_type, FaceClass.NEGATIVE, f'{counter:04}')
                elif iou >= 0.4 and iou <= 0.65 and part < pos + 1:
                    part += 1
                    preprocessing.save_img(
                        spic, output_path, model_type, sample_type, FaceClass.PART_FACE, f'{counter:04}')
                elif iou > 0.65:
                    pos += 1
                    preprocessing.save_img(
                        spic, output_path, model_type, sample_type, FaceClass.POSITIVE, f'{counter:04}')
                else:
                    continue
                counter += 1
        progbar.add(1)
